...
class LineSegment < GeometryValue
  ...
  def preprocess_prog
    if real_close(@x1, @x2) and real_close(@y1, @y2)
      Point.new(@x1, @y1)
    elsif real_close(@x1, @x2) and (@y1>@y1)
  ...
  def intersectPoint p
    if inbetween(p.x,@x1,@x2) and inbetween(p.y,@y1,@y2)
      p.intersectLine(two_points_to_line(@x1, @y1, @x2, @y2))
    else
      NoPoints.new()
    end
  end
  ...
  def intersectLine line
    res = two_points_to_line(@x1, @y1, @x2, @y2)
    if real_close(res.m, line.m)
      if real_close(res.b, line.b)
        LineSegment.new(@x1, @y1, @x1, @y2)
      else
        NoPoints.new()
      end
    else
      x = (line.b - res.b) / (res.m - line.m)
      y = res.m * x + res.b
  ...
  def intersectVerticalLine vline
    if inbetween(vline.x,@x1,@x2)
      vline.intersectLine(two_points_to_line(@x1, @y1, @x2, @y2))
    else
      NoPoints.new()
    end
  end
  ...
  def intersectLineSegment seg
    res1 = two_points_to_line(@x1, @y1, @x2, @y2)
    res2 = two_points_to_line(seg.x1, seg.y1, seg.x2, seg.y2)
    if real_close(res1.m, res2.m)
      if real_close(res1.b, res2.b)
        if real_close(@x1, seg.x1)
          if real_close(@x2, seg.x2)
            LineSegment.new(@x1, @y1, @x2, @y2) # inditical line segment
          elsif @x2<seg.x2
            LineSegment.new(@x1, @y1, @x2, @y2) # shorter line segment
          else
            LineSegment.new(@x1, @y1, seg.x2, seg.y2)
          end
        elsif @x1<seg.x1
          if real_close.new(@x2, seg.x1)
            Point(@x2, @y2) # one common point
          elsif @x2<seg.x1
            NoPoints()
          else # @x2>seg.x1
            if real_close(@x2, seg.x2)
      ...
      x = (res2.b - res1.b) / (res1.m - res2.m)
      y = res1.m * x + res1.b
      if inbetween(x,@x1,@x2) and inbetween(y,@y1,@y2) and inbetween(x, seg.x1, seg.x2) and inbetween(y, seg.y1, seg.y2)
        Point.new(x, y)
      else
        NoPoints.new()
      end
