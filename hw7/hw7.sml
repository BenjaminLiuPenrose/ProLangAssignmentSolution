...
datatype geom_exp =
	 ...
	 | Var of string
(*Q2
Add shift expressions as defined above to the SML implementation by adding the constructor
Shift of real * real * geom_exp to the definition of geom_exp and adding appropriate branches
to eval_prog and preprocess_prog.*)
...
(*Q2*)
	  ...
	  case e of
	  	NoPoints => e
	  	| Point (x, y) => eval_prog(Point (x+delta_x, y+delta_y), env)
	  	| Line (k, b) => eval_prog(Line (k, b+delta_y-delta_x*k), env)
	  	| VerticalLine x => eval_prog(VerticalLine (x+delta_x), env)
	  	| LineSegment (x1, y1, x2, y2) => eval_prog(LineSegment (x1+delta_x, y1+delta_y, x2+delta_x, y2+delta_y), env)
	  	| Intersect _ => eval_prog(Shift(delta_x, delta_y, eval_prog(e, env)), env)
	  	| Var _ => eval_prog(Shift(delta_x, delta_y, eval_prog(e, env)), env)
	  	| _ => raise Impossible "bad call to shift"
(* CHANGE: Add a case for Shift expressions *)

(* CHANGE: Add function preprocess_prog of type geom_exp -> geom_exp *)
(*Q1
Implement an SML function preprocess_prog of type geom_exp -> geom_exp to implement expres-
sion preprocessing as defined above. The idea is that evaluating program e would be done with
